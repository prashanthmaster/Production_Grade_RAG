...
> **Note:** `DATA/` and `processed_data/` are gitignored (not committed — datasets
> don't belong in source control). `DATA/true_data/` holds the real source
> documents the RAG pipeline should retrieve from; `DATA/noisy_data/` is a
> synthetic set of irrelevant documents (random academic-style PDFs/TXT/HTML/
> DOCX/PPTX) mixed in deliberately to test retrieval robustness — i.e. whether
> the pipeline correctly ignores noise and still surfaces the right answers
> from `true_data`. To reproduce locally, regenerate or source your own
> `noisy_data`/`true_data` folders under `DATA/` before running ingestion.
