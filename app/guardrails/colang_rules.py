...
YAML_CONTENT = """
models:
  # NOTE: this `main` entry is vestigial. app/guardrails/rails.py constructs
  # LLMRails(config, llm=guard_llm) with an explicit ChatOpenAI instance, and an
  # explicitly passed llm overrides whatever is declared here. Kept for clarity.
  - type: main
    engine: openai
    model: gpt-3.5-turbo

  # Pinned deliberately. NeMo embeds the Colang canonical forms above to match
  # user intent, and with no `embeddings` entry it silently picks a FastEmbed
  # default that can change between NeMo releases — which would change guardrail
  # behaviour on an unrelated dependency bump. Naming it here makes the model an
  # explicit contract, and lets Dockerfile.cloudrun pre-bake exactly this model
  # into the image so cold starts do not download it.
  - type: embeddings
    engine: FastEmbed
