...
class Settings(BaseSettings):
    ...
    def postgres_uri(self) -> str:
        ...
        Neon's hostname resolves to IPv6 addresses first; Cloud Run's default
        networking cannot route IPv6, so a plain hostname connection hangs
        until the client timeout. We resolve the hostname to an IPv4 address
        ourselves and pin the connection to it via `hostaddr`, while keeping
        `host` in the URI so TLS SNI still routes to the right Neon endpoint.
        """
        import socket
        ...
        if hostname:
            try:
                ...
                extra = f"{extra}&hostaddr={ipv4}"
            except OSError:
                # DNS resolution failed (e.g. offline dev environment) — fall
                # back to normal hostname resolution rather than hard-failing.
                pass
