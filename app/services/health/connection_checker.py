...
def _check_neon_postgres() -> ConnectionResult:
    ...
    t0 = time.monotonic()
    try:
        parts = urlsplit(settings.NEON_DB_URL)
        qs = parse_qs(urlsplit(settings.postgres_uri).query)
        hostaddr = qs.get("hostaddr", [None])[0]
        ...
        elapsed = time.monotonic() - t0
        logfire.warning(
            f"Postgres raw TCP connect FAILED to {target_host}:{target_port} "
            f"after {elapsed:.2f}s: {type(e).__name__}: {e}"
        )

    # --- Diagnostic step 1b: raw psycopg.connect(), bypassing the pool's
    # background-thread machinery, to see if the hang is in psycopg itself
    ...
    try:
        ...
        logfire.warning(f"Postgres health check failed: {type(e).__name__}: {e}")
