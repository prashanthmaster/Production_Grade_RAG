...
def _load_fallback():
    ...
    This import is deliberately lazy: `sentence-transformers` pulls in torch,
    roughly 1.9 GB of dependency that is pure dead weight whenever the Jina API
    is reachable. Keeping it lazy means the production container never pays for
    it on the happy path.

    The Cloud Run image (requirements-cloudrun.txt) ships WITHOUT torch, so this
    path raises ImportError there by design. That is the intended behaviour: in
    a container that must cold-start in seconds, silently falling back to a
    local transformer model would blow the startup budget and mask a
    misconfigured JINA_API_KEY. We want it loud.
    """
    try:
        from sentence_transformers import SentenceTransformer
    except ImportError as e:
        raise RuntimeError(
            "Local embedding fallback is unavailable: sentence-transformers/torch "
            "are not installed in this image (see requirements-cloudrun.txt). "
            "This almost always means JINA_API_KEY is missing, invalid, or the "
            "Jina API is unreachable — fix the key rather than installing torch. "
            "Check with: python -m app.services.health.connection_checker"
        ) from e
